...
def kick_user(group_id, user_id):
    # Use the token from the environment instead of message['token']
    membership_id = get_membership_id(group_id, user_id, GROUPME_TOKEN)
    if membership_id:
        return remove_member(group_id, membership_id, GROUPME_TOKEN)
    return False
    ...
    for phrase in FLAGGED_PHRASES:
        if phrase in (message.get('text') or '').lower():
            ...
            if kick_user(message['group_id'], message['user_id']):
                ...
                send('Kicked ' + message.get('name', 'user') + ' due to apparent spam post.', bot_id)
            else:
